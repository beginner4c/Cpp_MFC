...
BEGIN_MESSAGE_MAP(CXDrawerView, CView)
	...
	ON_COMMAND(ID_KITE1, &CXDrawerView::OnKite1)
	ON_COMMAND(ID_KITE2, &CXDrawerView::OnKite2)
	ON_COMMAND(ID_KITE3, &CXDrawerView::OnKite3)
...
void CXDrawerView::OnContextMenu(CWnd* /* pWnd */, CPoint point)
{
	...
	while (pos != NULL) {
		...
		if(ptr->ptlnRgn(testPoint.x, testPoint.y)){
			currentFigure = ptr;
			break;
		}
	}
